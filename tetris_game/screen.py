...
class Screen:
    ...
    def __init__(self, pygame, width, length, x, y, zoom):

        self.pygame = pygame
        self.display = self.pygame.display.set_mode((width, length))
    ...
    def update_screen(self, game):
        ...
        for i in range(definitions.n_rows):
        ...
        self.pygame.draw.rect(self.display, GRAY, [self.game_pos_x + self.zoom*(definitions.n_cols+0.5), self.game_pos_y, self.zoom*6, self.zoom*6], 1)
        text_next = self.text_font.render("Next", True, BLACK)
        self.display.blit(text_next, [self.game_pos_x + self.zoom*(definitions.n_cols+2.5), self.game_pos_y])

        for i in range(4):
            for j in range(4):
                self.pygame.draw.rect(self.display, GRAY, [self.game_pos_x + self.zoom*(j+definitions.n_cols+1.5), self.game_pos_y+self.zoom*(i+1), self.zoom, self.zoom], 1)
        if game.next_piece is not None:
            for block in game.next_piece.image():
                ...
                self.pygame.draw.rect(self.display, colors[game.next_piece.type],
                    [self.game_pos_x + self.zoom*(j+definitions.n_cols+1.5)+1,
                     self.game_pos_y + self.zoom*(i+1)+1,
                     self.zoom-2, self.zoom-2])

        self.pygame.draw.rect(self.display, GRAY, [self.game_pos_x + self.zoom*(definitions.n_cols+0.5), self.game_pos_y+self.zoom*(7), self.zoom*6, self.zoom*8], 1)
        ...
        self.display.blit(text_score, [self.game_pos_x + self.zoom*(definitions.n_cols+1), self.game_pos_y+self.zoom*(7)+self.text_font_size*0.5])
        self.display.blit(text_lines, [self.game_pos_x + self.zoom*(definitions.n_cols+1), self.game_pos_y+self.zoom*(7)+self.text_font_size*2])
        self.display.blit(text_pieces, [self.game_pos_x + self.zoom*(definitions.n_cols+1), self.game_pos_y+self.zoom*(7)+self.text_font_size*3.5])
