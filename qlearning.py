...
class qlearning():
    ...
    def calculate_target_q(self, next_state):
        ...
        if gameover:
            ...
            with torch.no_grad():
                return [math.tanh(self.delta*self.rewards_object.total_reward(lines, action[0], gameover) + self.gamma*self.player.stable_model(afterstate, next_piece).detach().numpy()[0])]
